...
function parse(html,base){
 ...
 const cards=scope.match(/<div\b[^>]*data-offer-card\b[^>]*>[\s\S]*?(?=<div\b[^>]*data-offer-card\b[^>]*>|$)/gi)||[];
 for(const cardHtml of cards){
  const link=cardHtml.match(/<a\b[^>]*href=["']([^"']+)["'][^>]*data-track=["']offer-link["'][^>]*>([\s\S]*?)<\/a>/i);if(!link)continue;
  const anchorText=clean(link[2]);if(!/Dom na (?:sprzedaż|wynajem)/i.test(anchorText))continue;
  const url=abs(link[1],base);if(!url||seen.has(url))continue;
  const card=clean(cardHtml);
  const priceMatch=card.match(/([0-9][0-9\s\u00a0.]*)\s*zł/i),areaMatch=card.match(/([0-9]+(?:[,.][0-9]+)?)\s*m(?:²|2)/i);if(!priceMatch||!areaMatch)continue;
