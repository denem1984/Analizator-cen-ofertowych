...
function offerKey(o){const price=Number(o.price),area=Number(o.area);if(!Number.isFinite(price)||!Number.isFinite(area))return'';const locality=normText(o.locality||o.location),street=normText(o.street),title=normText(o.title);if(street)return `addr|${locality}|${street}|${price}|${area}`;if(title&&title.length>8)return `title|${title}|${price}|${area}`;return''}
