...
function normText(v){return String(v||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/ł/g,'l').replace(/[^a-z0-9]+/g,' ').trim().replace(/\s+/g,' ')}
function offerKey(o){const price=Number(o.price),area=Number(o.area);if(!Number.isFinite(price)||!Number.isFinite(area))return'';const locality=normText(o.locality||o.location),street=normText(o.street);const title=normText(o.title);if(street)return `addr|${locality}|${street}|${price}|${area}`;if(title&&title.length>8)return `title|${title}|${price}|${area}`;return''}
function dedupeOffers(offers){const seenUrl=new Map(),seenKey=new Map(),duplicates=[],result=[];for(const offer of offers){const urlKey=normalizeUrl(offer.url),dataKey=offerKey(offer);let reason='',kept=null;if(urlKey&&seenUrl.has(urlKey)){reason='ten sam URL';kept=seenUrl.get(urlKey)}else if(dataKey&&seenKey.has(dataKey)){reason='ta sama lokalizacja/adres + cena + powierzchnia';kept=seenKey.get(dataKey)}if(reason){duplicates.push({reason,kept,duplicate:offer});continue}if(urlKey)seenUrl.set(urlKey,offer);if(dataKey)seenKey.set(dataKey,offer);result.push(offer)}return{rows:result,duplicates}}
...
async function runApartmentDiagnostic({location='Olsztyn',area=62,tolerance=10,radius=0}={}){
 ...
 return{version:'0.7.2-apartment-diagnostics',propertyType:'mieszkanie',inputLocation:location,resolvedLocation:resolved,location:canonicalLocation,wojewodztwo:resolved.wojewodztwo||'',area:target,tolerance:tol,minArea,maxArea,radius:requestedRadius,beforeCrossDedup:before.length,unique:cross.rows.length,duplicatesRemoved:cross.duplicates.length,duplicates:cross.duplicates,sources,offers:cross.rows,diagnostics:{totalRecognized:sources.reduce((a,s)=>a+Number(s.recognized||0),0),totalComplete:sources.reduce((a,s)=>a+Number(s.complete||0),0),totalFiltered:before.length,totalFinal:cross.rows.length,rejectionByPortal:Object.fromEntries(sources.map(s=>[s.portal,s.diagnostics]))}};
}
